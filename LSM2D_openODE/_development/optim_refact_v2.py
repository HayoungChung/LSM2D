...
class OptimRefact(object):
    ...
    def get_phis(self, phi):
        ...
        self.phi = phi
        u, boundary = self.cant.get_u(self.phi)
	    ...
	    BoundaryPoints[ii, 1] = boundary.points[ii].coord.y
        ...
        self.constraintDistances[0] = (self.cant.lxy[0]*self.cant.lxy[1] * 0.5 - boundary.area)
        #print("constraints = %d"  % self.constraintDistances[0])

        myOptim = optim.Optimise(self.cant.levelSet, boundary.points, \
               self.constraintDistances, lambdas, self.Multipliers, 0.9, False)
        # myOptim.preprocess()
        ...
        for ii in range(0,self.cant.mesh.nNodes):
            ...
            dphi_dt[ii] = -myOptim.dphi_dt[ii]*lambdas[0];
    ...
    def get_u(self, phi):
        ...
        for ii in range(0,nELEM):
            if self.mesh.elements[ii].area < 1e-6: #AllowedAreaFraction
                ...
                self.CMesh.AreaFraction[ii] = self.mesh.elements[ii].area
	print(LA.norm(self.CMesh.AreaFraction))
         # print ("Assembling ...")
