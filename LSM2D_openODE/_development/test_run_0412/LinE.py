...
class FEAMeshQ4(object):
    ...
    def __init__(self, *args):
        ...
        #self.get_Area() # computes initial area
#        self.IntegrationPoints = np.ones(self.nELEM)*4
...
#        self.sK = sp.sparse.csr_matrix((int(nDOF),int(nDOF)),dtype=np.float64)
        self.F  = np.zeros(nDOF)

    def Assembly(self):
        nDOF = self.CMesh.nNODE*self.CMesh._dpn
#        self.sK = sp.sparse.csr_matrix((int(nDOF),int(nDOF)),dtype=np.float64)
#        K_lil = sp.sparse.lil_matrix((nDOF,nDOF),dtype=np.float64)
        ...
        for ee in range(0,self.CMesh.nELEM):
            ...
            for gg in range(0,len(wi)):
                ...
                # LF += matB.transpose().dot(self.Cijkl).dot(matB)*Jw
            
            # this is a global K matrix
#            K_lil[elem_dof,elem_dof] += LK
...
#        self.sK = K_lil.tocsr()
#            self.sK[np.ix_(elem_dof,elem_dof)] += LK # warning msg. as this is slow
            # F[elem_dof-1)] += LF
    ...
    def set_F(self,*args):#DoF_id,value):
#        if not('self.F' in locals()):
#            print("??")
#            self.F = np.zeros(self.CMesh.Nodes.__len__()*self.CMesh._dpn)
        if len(args)==2:
            DoF_id = args[0]
            value = args[1]
    ...
    def solve(self):
        if self.isBC == False:
            ...
            #sp.sparse.linalg.spsolve(self.sK,self.F)
            #self.Field = sp.linalg.solve(self.sK,self.F)
