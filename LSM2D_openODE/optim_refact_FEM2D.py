...
class OptimRefact(object):
    def __init__(self, *args):
        ...
        ex = nx-1
        ey = ny-1
        self.FEM = PyFEMSolver(nx,ny,nx-1,ny-1,1,0.3)

        self.LSM = pyLevelset(nx,ny, 0.5)
        
        self.phi = np.zeros(nx*ny)
        self.LSM.get_phi(self.phi)
        print(np.sum(self.phi))

        self.areafraction = np.zeros(ex*ey)
        self.LSM.get_area_fractions(self.areafraction)
        self.nNodes = nx*ny
        self.nElems = ex*ey
    def get_delphi(self,phi):
        self.phi = phi.reshape(self.nNodes)

        self.LSM.set_signedDistance(self.phi) # reinitialization...
        self.LSM.get_area_fractions(self.areafraction)
        self.FEM.set_area_fractions(self.areafraction)
        
        self.FEM.get_stiffness_matrix()
