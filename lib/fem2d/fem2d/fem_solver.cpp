...
void FEMSolver::get_sensitivity_LSTO(double *u, double *xpos, double *ypos, double *sens)
{
  ...
  vector<double> ri = {-1. / sqrt(3.0), +1. / sqrt(3.0), +1. / sqrt(3.0), -1. / sqrt(3.0)};
  vector<double> si = {-1. / sqrt(3.0), -1. / sqrt(3.0), +1. / sqrt(3.0), +1. / sqrt(3.0)};
  Vector x0, y0, u0;
  // Vector tmp_;
  double N1, N2, N3, N4;
  // vector<double> wi = {1, 1, 1, 1};
  ...
  for (int ii = 0; ii < num_nodes_x - 1; ii++)
  {
    for (int jj = 0; jj < num_nodes_y - 1; jj++)
    {
      for (int gg = 0; gg < 4; gg++)
      {
        ...
        if (gg == 0)
        {
          Vector tmp_ = dot(Ke0, u0);
          // tmp_ = tmp_ * area_fraction[ii][jj];
          sens[index] = dot(u0, tmp_) * area_fraction[ii][jj];
          sens[index] /= wj;
        }
        else if (gg == 1)
        {
          Vector tmp_ = dot(Ke1, u0);
          //tmp_ = tmp_ * area_fraction[ii][jj];
          sens[index] = dot(u0, tmp_) * area_fraction[ii][jj];
          sens[index] /= wj;
        }
...
void FEMSolver::set_area_fractions(const double *areafraction)
{
