...
void Boundary::LaplacianSensitivityFilter(int which_constraint)
{
    // Defining which objective or constraint to filter sensitivities.
    int nfilter = which_constraint;

    // Neighbours variables.
    int neig0, neig1;

    // Vector of filtered sensitivities.
    std::vector<double> filteredSens_n0;
    filteredSens_n0.resize(nPoints);
    std::vector<double> filteredSens_n1;
    filteredSens_n1.resize(nPoints);

    // // Computing maximum absolute sensitivity value.
    // double maxabsSens = 0.0;
    // for (int i = 0; i < nPoints; i++)
    // {
    //     if (std::abs(points[i].sensitivities[nfilter]) > maxabsSens)
    //     {
    //         maxabsSens = std::abs(points[i].sensitivities[nfilter]);
    //     }
    // }

    // // Computing vector of oscillating sensitivity points.
    // std::vector<int> oscPoints;
    // for (int i = 0; i < nPoints; i++)
    // {
    //     // Only for elements with two neighbours.
    //     if (points[i].nNeighbours == 2)
    //     {
    //         // If they have oscillating sensitivity signs with their neighbours:
    //         if ((points[points[i].neighbours[0]].sensitivities[nfilter] < 0) & (points[points[i].neighbours[1]].sensitivities[nfilter] < 0))
    //         {
    //             if (points[i].sensitivities[nfilter] > 0)
    //             {
    //                 // Storing oscillating point.
    //                 oscPoints.push_back(i);
    //             }
    //         }
    //         else if ((points[points[i].neighbours[0]].sensitivities[nfilter] > 0) & (points[points[i].neighbours[1]].sensitivities[nfilter] > 0))
    //         {
    //             if (points[i].sensitivities[0] < 0)
    //             {
    //                 // Storing oscillating point.
    //                 oscPoints.push_back(i);
    //             }
    ...
    //     // Initialising filtered sensitivities as current sensitivities.
    //     filteredSens_n0[i] = points[i].sensitivities[nfilter];
    // }

    // // Computing critical oscillating points (only to use as stop criterion for Laplacian smoothing).
    // std::vector<int> critPoints;
    // for (int i = 0; i < oscPoints.size(); i++)
    // {
    //     // If oscillating point is signifcant (tol = 1e-3) relatively to the maxabsSens.
    //     if (std::abs(points[oscPoints[i]].sensitivities[nfilter]) >= maxabsSens*0.001)
    //     {
    //         // Store critical point.
    //         critPoints.push_back(oscPoints[i]);
    //     }
    // }
    // oscPoints.clear();
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        for (int i = 0; i < nPoints; i++)
        {
            if (points[i].nNeighbours == 2)
            {
                ...
                neig0 = points[i].neighbours[0];
                neig1 = points[i].neighbours[1];

                // Applying Laplacian smoothing.
                filteredSens_n1[i] = filteredSens_n0[i] + 0.5*(0.5*(filteredSens_n0[neig0]-filteredSens_n0[i])+0.5*(filteredSens_n0[neig1]-filteredSens_n0[i]));
            }
            else if (points[i].nNeighbours == 1)
            {
                // Getting neighbour points.
                neig0 = points[i].neighbours[0];

                // Applying Laplacian smoothing.
                filteredSens_n1[i] = filteredSens_n0[i] + 0.5*(filteredSens_n0[neig0]-filteredSens_n0[i]);
            }
            else
            {
        ...
        for (int i = 0; i < nPoints; i++)
        {
            points[i].sensitivities[nfilter] = filteredSens_n1[i];
            filteredSens_n0[i] = filteredSens_n1[i];
            filteredSens_n1[i] = 0.0;
        }
    }
}
