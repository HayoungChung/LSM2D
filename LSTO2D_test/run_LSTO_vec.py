...
for i_HJ in range(0,max_loop):
    ...
    fixedGpts_sens = sens_compl.reshape((num_elems,Order_gpts**2))#[:,(2,3,0,1)]
    ...
    for qq in range(0,4):
	tmp_v = fixedGpts_sens[:,qq].reshape((80,160))
	Gdot[:,qq] = tmp_v.transpose().flatten()
	tmp_v = xpos[:,qq].reshape((80,160))
	xdot[:,qq] = tmp_v.transpose().flatten()
	tmp_v = ypos[:,qq].reshape((80,160))
