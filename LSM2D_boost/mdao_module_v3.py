...
class LSM_M2DO(Component):
    def __init__(self, exy, *args):
        ...
        self.add_param('BoundaryPoints', val = self.boundary.points)
    ...
    def solve_nonlinear(self, params, unknowns, resids):
        ...
        BoundaryPointCoords = np.zeros([nBpts,2])
    
        for ii in range(0,nBpts):
            BoundaryPointCoords[ii,0] = BoundaryPoints[ii].coord.x
            BoundaryPointCoords[ii,1] = BoundaryPoints[ii].coord.y #VERIFIED
        
        unknowns['BoundaryPointCoords'] = BoundaryPointCoords
        

class Optimiser_pre(Component):
    def __init__(self, optimClass, BoundaryPointsStruct):
        super(Optimiser_pre,self).__init__()
        self.optimise = optimClass
        self.boundarypoints = BoundaryPointsStruct
        self.add_param('BoundaryPointSensitivities', val = np.zeros([len(BoundaryPointsStruct),1], pass_by_obj = True))
...
class Optimiser_post(Component):
    def __init__(self, optimClass, BoundaryPointsStruct):
        super(Optimiser_post,self).__init__()
        self.optimise = optimClass
        self.add_param('lambdas', val = np.zeros(2))
        self.add_output('BoundaryPoints', val = BoundaryPointsStruct, pass_by_obj = True)
    ...
    def solve_nonlinear(self, params, unknowns, resides):
        ...
        unknowns['BoundaryPoints'] = self.BoundaryPointsStruct
