...
class Sensitivity_M2DO(Component):
    def __init__(self, CLinElasticity,nBpts,*args):
        ...
        # from slsm
        self.add_param('BoundaryPointCoords', shape = (nBpts,2))
        self.add_param('AreaFraction',val = np.zeros(CLinElasticity.CMesh.nELEM))

        # to optimization
        self.add_output('BoundaryPointSensitivities', val = np.zeros(nBpts))
...
class Optim_pre_(Component):
    ...
    def __init__(self,moveLimit, Bpts, *args):
        ...
        self.add_param('BoundaryPoints', val = Bpts)
        self.add_param('BoundaryPointSensitivities', val = np.zeros(len(Bpts)) )
        
        m = vector__double__()
        m.append(0)
        self.add_param('ConstraintDistance', val = m)
    ...
    def solve_nonlinear(self,params, unknowns,resids):
        ...
        for ii in range(0,boundarypoints.__len__()):
            boundarypoints[ii].sensitivities[0] = boundarypointsSensitivites[ii]
            boundarypoints[ii].sensitivities[1] = -1
        ...
        self.negLimit = [b.negativeLambdaLimits[0],b.negativeLambdaLimits[1]]
        self.posLimit = [b.positiveLambdaLimits[0],b.positiveLambdaLimits[1]]
