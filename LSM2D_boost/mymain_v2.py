...
boundary.discretise(False) # boundary.coordinate
boundary.computeAreaFractions() # mesh.AreaFraction
boundary.ComputeNormalVectors() # normalvectors
...
#    t = clock_t.time()
    ...
    def objF(x):
        ...
        m.append(x[0])
        m.append(x[1])
        return b.callback(m,vector__double__(),0)
        
    def conF(x):
        m = vector__double__()
        m.append(x[0])
        m.append(x[1])
        return b.callback(m,vector__double__(),1)
        
    cons = ({'type' : 'eq', 'fun': lambda x: conF(x)})
    res = sp_optim.minimize(objF, [lambdas[0],lambdas[1]], method='SLSQP', options={'disp': False}, \
                   bounds=((b.negativeLambdaLimits[0],b.positiveLambdaLimits[0]),(b.negativeLambdaLimits[1],b.positiveLambdaLimits[1])),\
                   constraints = cons)
    lambdas[0] = res.x[0]
    lambdas[1] = res.x[1]
    #%%
    res2 = b.postprocess(res.fun)       
