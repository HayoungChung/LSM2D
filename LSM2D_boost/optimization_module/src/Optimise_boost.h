...
public:
     ...
     */
    Optimise(slsm::LevelSet& levelSet_, std::vector<slsm::BoundaryPoint>& boundaryPoints_, const std::vector<double>& constraintDistances_,
        std::vector<double>& lambdas_, std::vector <double>& Multipliers_, 
        // double&, 
        double maxDisplacement_ = 0.5, bool isMax_ = false,
    ...
    // WIP = exposed to newer lib
    unsigned int num_bpts; // number of boundary points
    std::vector<double> phi;
    std::vector<double> velocity;
    std::vector<double> dphi_dt;
    void compute_dphi_dt(double);
    // ==================
    double solve();
    std::vector<double> get_lambdas();
    void set_lambdas(const double, const double);
    void preprocess();
    double postprocess(double optObjective_);
    double nlopt_activate();
