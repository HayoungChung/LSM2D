...
{
    num_bpts = levelSet.signedDistance.size();
    velocity.resize(num_bpts);
    phi.resize(num_bpts);
    dphi_dt.resize(num_bpts);
...
void Optimise::compute_dphi_dt(){
    ...
    for (unsigned int i = 0 ; i < num_bpts; ++i){
        velocity[i] = levelSet.velocity[i];
        phi[i] = levelSet.signedDistance[i];
    ...
    dphi_dt.resize(phi.size());
    for (unsigned int i = 0; i < nPoints; i++){
        dphi_dt[i] = (levelSet.gradient[i]*boundaryPoints[i].velocity)
    }
...
BOOST_PYTHON_MODULE(Opt_Module){
    ...
    class_<optim::Optimise>("Optimise", init<std::vector<slsm::BoundaryPoint>&, std::vector<double>&, 
