...
void MA57Solver :: pyCompute (int n, int nz, int* irn, int* jcn, double* a) {
	// n = shape, nz = nonzeros
	this->n = n;
	...
	ma57ad_(&n, &nz, irn, jcn, &lkeep, keep.data(), iw.data(), icntl.data(), info.data(), rinfo.data()) ;
    ...
    ma57bd_(&n, &nz, a, fact.data(), &lfact, ifact.data(), &lifact, &lkeep, keep.data(), iw.data(), icntl.data(), cntl.data(), info.data(), rinfo.data()) ;
}
...
void MA57Solver:: pySolve(double* x){
	/*
	Solve that mofo:
	*/
	int job = 1 ; 	  // if job <= 1, solves Ax = b.
	...
	ma57cd_(&job, &n, fact.data(), &lfact, ifact.data(), &lifact, &nrhs, x, &lrhs, w.data(), &lw, iw.data(), icntl.data(), info.data()) ;
...
void MA57Solver :: compute(SparseMatrix<double> & A) {
	...
	*/
	...
	pyCompute(n, nz, irn.data(), jcn.data(), a.data());
}
