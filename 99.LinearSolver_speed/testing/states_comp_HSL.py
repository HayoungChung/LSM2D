...
class StatesComp(ImplicitComponent):
    ...
    def setup(self):
        ...
        self.declare_partials('states', 'rhs', val=data, rows=arange, cols=arange) # picking up displacement from rhs
    ...
    def _solve(self, sol, rhs, mode): # WIP: get solution from rhs (force, matrix)
        if mode == 'fwd':
            arg = 'N' # solve Ax = b
        elif mode == 'rev':
            arg = 'T'
